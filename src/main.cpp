...
const char *LUNAR_MONTG[] = {"正月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","腊月"};

const char *WEEKDAY_CN[] = {"周日", "星期一", "星期二", "星期三", "星期四", "星期五", "周六"};
...
void DrawMultiLineEString(string content, uint16_t x, uint16_t y, uint16_t contentAreaWidthWithMargin, uint16_t lineHeight)
{
  string ch;
  vector<string> contentArray;
  for (size_t i = 0, len = 1; i != content.length(); i += len)
  {
    unsigned char byte = (unsigned)content[i];
    if (i == 27 || i == 27*2-1 || i == 27*3-2 || i == 27*4-3)
    {
      if ((unsigned char)content[i] != 0x20 && (unsigned char)content[i+1] != 0x20)
      {
  ...
  for (size_t j = 0; j < contentArray.size(); j++)
  {
    outputContent += contentArray[j];
    int16_t wTemp = u8g2Fonts.getUTF8Width(outputContent.c_str());
    if (j != 0 && (j % 27 == 0 || j == (contentArray.size() - 1)))
    {
      u8g2Fonts.drawUTF8(x, y, outputContent.c_str());
      y += lineHeight;
      outputContent = "";
...
void ShowPageHeader()
{
  ...
  u8g2Fonts.drawUTF8(48, 64 + 24, WEEKDAY_CN[DateTime.getParts().getWeekDay()]);
...
void ShowCountDown()
{
  String year = String(DateTime.getParts().getYear()) + "年仅剩";
  String dateInCenter = String(DaysLeft());
  String percentLeft = String(PercentLeft()*100) + "% left";

  u8g2Fonts.setFont(u8g2_mfyuehei_14_gb2312);
  int16_t titleWidth = u8g2Fonts.getUTF8Width(year.c_str());
  u8g2Fonts.drawUTF8((DISPLAY_WIDTH - titleWidth) / 2, 180, year.c_str());

  u8g2Fonts.drawUTF8((DISPLAY_WIDTH - 100), 300, "天");
  ...
  u8g2Fonts.setFont(u8g2_mfyuehei_14_gb2312);
  int16_t percentWidth = u8g2Fonts.getUTF8Width(percentLeft.c_str());
  u8g2Fonts.drawUTF8((DISPLAY_WIDTH - percentWidth) / 2, 340, percentLeft.c_str());
}
...
void ShowQuotes()
{
  u16_t r = random(QuotesCount);
  const char *quote = Quotes[r];
  Serial.printf("pick %u: %s\n", r, quote);
  u8g2Fonts.setFont(u8g2_mfyuehei_18_gb2312);
  DrawMultiLineEString(string(quote), 80, 420, 300, 36);
}
...
void setup()
{
  ...
  Serial.begin(115200); // 打开串口，设置波特率为 115200 bps
  Serial.println();
